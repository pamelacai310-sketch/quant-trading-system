...
def _new_open_trade(row: pd.Series, side: str, quantity: float, execution_price: float) -> Dict[str, Any]:
    return {
        ...
        "high_since_entry": float(execution_price),
        "low_since_entry": float(execution_price),
...
def _trailing_exit_triggered(open_trade: Optional[Dict[str, Any]], row: pd.Series, limits: Dict[str, Any]) -> bool:
    ...
    favorable = (entry - float(open_trade["low_since_entry"])) / max(entry, 1e-9)
    if favorable >= activation:
        open_trade["trailing_active"] = True
    return bool(open_trade["trailing_active"] and close / max(float(open_trade["low_since_entry"]), 1e-9) - 1.0 >= trail)
...
def _close_open_trade(
    ...
    if side == "long":
        ...
        mae_pct = ((entry - float(open_trade["high_since_entry"])) / max(entry, 1e-9)) * 100.0
        mfe_pct = ((entry - float(open_trade["low_since_entry"])) / max(entry, 1e-9)) * 100.0
        final_pnl_pct = ((entry - float(execution_price)) / max(entry, 1e-9)) * 100.0
...
def backtest_strategy(
    ...
    # Signals use completed bar t; fills use the next bar's open.
    # This bar simulator does not claim queue, limit-fill or exchange-rule fidelity.
    if starting_cash <= 0 or fee_bps < 0 or slippage_bps < 0:
        raise ValueError("Invalid capital or costs")
    for column in ("open", "high", "low", "close"):
        if column not in enriched or not np.isfinite(enriched[column]).all() or (enriched[column] <= 0).any():
            raise ValueError(f"Invalid or missing {column}")
    if not pd.to_datetime(enriched.timestamp).is_monotonic_increasing or enriched.timestamp.duplicated().any():
        raise ValueError("Bars must have unique increasing timestamps")
    futures = spec.get("asset_type") == "futures"
    multiplier = float(spec.get("contract_multiplier", 1.0))
    if multiplier <= 0 or (futures and "contract_multiplier" not in spec):
        raise ValueError("Futures require explicit positive contract_multiplier")
    if futures and not 0 < float(spec.get("margin_rate", 0)) <= 1:
        raise ValueError("Futures require explicit margin_rate")
    ledger = PositionLedger(starting_cash, multiplier, futures)
    direction = spec.get("direction", "long_only")
    sizing = spec.get("position_sizing", {})
    max_units = float(sizing.get("max_units", 100))
    fraction = float(sizing.get("risk_fraction", 0.15))
    if not 0 <= fraction <= 1 or max_units < 0:
        raise ValueError("Invalid position sizing")
    lot = float(sizing.get("lot_size", 1 if futures else 0.000001))
    if lot <= 0:
        raise ValueError("lot_size must be positive")
    risk_limits_used = _resolve_adaptive_risk_limits(spec)
    open_trade = None
    closed_trades, trades = [], []
    equity_curve = [dict(timestamp=enriched.iloc[0]["timestamp"], equity=starting_cash,
                         cash=starting_cash, position_qty=0.0, close=float(enriched.iloc[0]["close"]))]
    for idx in range(1, len(enriched)):
        decision, row = enriched.iloc[idx-1], enriched.iloc[idx]
        qty = ledger.quantity
        signal = int(np.sign(qty))
        reason = "hold"
        if qty:
            pnl = np.sign(qty) * (float(decision["close"]) / ledger.average_price - 1)
            rules = spec.get("exit_rules" if qty > 0 else "short_exit_rules", [])
            if rules and all(_condition(enriched, idx-1, c) for c in rules):
                signal, reason = 0, "rule_exit"
            stop, take = risk_limits_used["stop_loss_pct"], risk_limits_used["take_profit_pct"]
            if stop is not None and pnl <= -stop:
                signal, reason = 0, "stop_loss"
            if take is not None and pnl >= take:
                signal, reason = 0, "take_profit"
            if _trailing_exit_triggered(open_trade, decision, risk_limits_used):
                signal, reason = 0, "trailing_stop"
        else:
            for side, key in ((1, "entry_rules"), (-1, "short_entry_rules")):
                rules = spec.get(key, [])
                if side < 0 and direction != "long_short":
                    continue
                if rules and all(_condition(enriched, idx-1, c) for c in rules):
                    signal, reason = side, "enter_long" if side > 0 else "enter_short"
                    break
        # Preserve entry units until exit. No undocumented daily rebalancing.
        target = qty if signal else 0.0
        price = float(row["open"])
        if signal and not qty:
            execution = price * (1 + slippage_bps / 10000 * signal)
            budget = max(0.0, ledger.equity(price)) * fraction
            units = min(max_units, budget / (execution * multiplier * (1 + fee_bps / 10000)))
            target = signal * math.floor(units / lot) * lot
        delta = target - qty
        if abs(delta) > 1e-9:
            execution = price * (1 + slippage_bps / 10000 * np.sign(delta))
            fee = abs(delta) * execution * multiplier * fee_bps / 10000
            realized = ledger.fill(delta, execution, fee)
            if realized is not None:
                # Exit at open: do not include the exit bar's later high/low.
                open_trade["high_since_entry"] = max(open_trade["high_since_entry"], execution)
                open_trade["low_since_entry"] = min(open_trade["low_since_entry"], execution)
                closed = _close_open_trade(open_trade, row, execution, reason)
                closed.update(realized)
                closed["final_pnl_pct"] = 100 * realized["net_pnl"] / (realized["quantity"] * realized["entry_price"] * multiplier)
                closed_trades.append(closed)
                open_trade = None
            elif ledger.quantity:
                open_trade = _new_open_trade(row, "long" if target > 0 else "short", target, execution)
            trades.append(dict(timestamp=row["timestamp"], decision_timestamp=decision["timestamp"],
                               side="buy" if delta > 0 else "sell", quantity=abs(delta),
                               price=execution, fee=fee, signal=signal, reason=reason))
        _update_open_trade_path(open_trade, row)
        equity = ledger.equity(float(row["close"]))
        if equity <= 0:
            raise ValueError("Account insolvent: reject performance inference")
        equity_curve.append(dict(timestamp=row["timestamp"], equity=equity, cash=ledger.cash,
                                 position_qty=ledger.quantity, close=float(row["close"])))
        if futures and abs(ledger.quantity) * float(row["close"]) * multiplier * float(spec["margin_rate"]) > equity:
            raise ValueError("Margin breach: unsupported liquidation; reject backtest")
    equity_series = pd.Series([item["equity"] for item in equity_curve], dtype=float)
    returns = equity_series.pct_change().fillna(0.0)
    # Open positions remain unrealized and are excluded from closed-trade statistics.
    ...
    result = BacktestResult(
        ...
        stats={
            ...
            "execution_model": "prior_close_signal_next_open_fixed_units",
            "accounting_mode": "futures_mark_to_market" if futures else "cash_security",
