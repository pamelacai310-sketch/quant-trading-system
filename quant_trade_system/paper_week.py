"""Live-quote paper execution of the frozen P0 daily model. No broker access.
...
DIR=ROOT/'experiments/paper_week_20260907'
...
def parse_quotes(raw,config):
    ...
    for item in config['instruments']:
        symbol=item['symbol'];key=('sh' if item['type']=='stock' else 'nf_')+symbol
        match=re.search(r'var hq_str_'+re.escape(key)+r'="([^"]*)";',raw)
        if not match:raise ValueError(f'Missing quote {symbol}')
        v=match[1].split(',')
        try:
            if item['type']=='stock':
        ...
        except (IndexError,ValueError) as exc:raise ValueError(f'Malformed quote {symbol}: {exc}') from exc
...
def fetch_quotes(config):
    keys=[('sh' if i['type']=='stock' else 'nf_')+i['symbol'] for i in config['instruments']]
...
def freeze(directory=DIR):
    ...
    bootstrap=read(ROOT/'experiments/p0_week_20260907/observations/2026-09-04.json')
    if bootstrap['captured_at']>=config['start_at']:raise ValueError('Late bootstrap')
    write_once(directory/'bootstrap.json',{'source_observation_hash':digest(bootstrap),'inputs':bootstrap['inputs']})
    write_once(path,dict(frozen_at=clock().isoformat(),protocol_hash=digest(config),code_hash=code_digest(config),bootstrap_hash=digest(read(directory/'bootstrap.json'))))
...
def initial_state(config,bootstrap):
    ...
    return dict(states=states,orders={},marks={},cycles={},closed_trades=[],bought_today={},last_fill_quote={},
                last_model_date=config['bootstrap_date'],model_inputs=bootstrap['inputs'],peak_equity={s:config['cash_per_sleeve']*len(config['instruments']) for s in states},halted=False)


def queue_orders(config,state,now):
    targets=make_orders(config,state['model_inputs'],state['states']);orders={}
    for strategy,items in targets.items():
        orders[strategy]={}
        for symbol,t in items.items():
            order={**t,'created_at':now.isoformat(),'strategy':strategy,'symbol':symbol,'status':'pending'}
...
def execute_quotes(config,state,quotes,now):
    fills=[];rejects=[]
    for item in config['instruments']:
        symbol=item['symbol'];quote=quotes[symbol];ts=datetime.fromisoformat(quote['timestamp'])
        ...
        for strategy in state['states']:
            ...
            amount=min(abs(delta),math.floor(size/item['lot'])*item['lot'])
...
def step(directory=DIR):
    ...
    if now>=end:
        ...
        data={i['symbol']:fetch_bars(i,str(now.date())) for i in config['instruments']}
        state['model_inputs']=data;state['last_model_date']=str(now.date())
        # Closing marks are valuations only, never retrospective fills.
        for symbol,payload in data.items():state['marks'][symbol]={'price':payload['bars'][-1]['close'],'quote_time':str(now.date())+'T15:00:00+08:00','source':'daily_close_valuation_only'}
...
def report(directory=DIR):
    ...
    initial=config['cash_per_sleeve']*len(config['instruments']);result={}
    for strategy,items in state['states'].items():
        ...
        result[strategy]=dict(starting_cash=initial,ending_nav=nav,net_profit=nav-initial,net_return=nav/initial-1,
    ...
    output=dict(experiment=config['experiment_id'],as_of=clock().isoformat(),start_at=config['start_at'],end_at=config['end_at'],
                ...
                repeatable_advantage_proven=False,notes=['Net NAV includes unrealized PnL; open positions are not closed wins.','Payoff ratio is mean winning net PnL / absolute mean losing net PnL; null if either group is empty.','Display-book simulated fills and declared costs; not actual broker execution.'])
